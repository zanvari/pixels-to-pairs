"""CORD prompt construction used by the benchmark."""
...
Task: Extract ALL key–value pairs from the given receipt OCR text.

Definition:
- key = the full field label as written in the document.
- value = the associated content for that field.

Important extraction rules:
- Keep multi-word field labels together as ONE key.
  Example: "SUB TOTAL" must be one key, not separate keys "SUB" and "TOTAL".
- Do NOT include leftover pieces of a field label in the value.
  Example: if the field is "CHANGE DUE", do not put "DUE" in the value.
- Do NOT treat item quantities such as x1, x2, 2x, etc. as keys.
- Do NOT use pure numbers or monetary amounts as keys.
- For a pattern such as "ITEM_NAME 18,000", use:
  key = "ITEM_NAME"
  value = "18,000"
...
def load_cord_examples(
    example_files_dir: Path,
) -> Dict[str, Dict[str, str]]:
    """Load the three frozen CORD few-shot demonstrations."""
...
def _ensure_examples(
    ...
    if example_files_dir is None:
        raise ValueError(
            "example_files_dir is required for CORD "
            "few-shot prompts."
        )

    return load_cord_examples(
...
def build_prompt(
    ...
    *,
    ...
    """Build a CORD prompt for the requested shot condition."""
    ...
    if shot_variant == "2shot":
        return build_prompt_2shot(
            doc_text,
            examples,
        )
    if shot_variant == "3shot":
        return build_prompt_3shot(
            doc_text,
            examples,
        )
